...
export function WorkWorkspace({ inlineApprovalsEnabled = false, automaticAttentionEnabled = false, voiceEnabled = false, automaticVoiceEnabled = false, liveVoiceEnabled = false }: { inlineApprovalsEnabled?: boolean; automaticAttentionEnabled?: boolean; voiceEnabled?:boolean; automaticVoiceEnabled?:boolean; liveVoiceEnabled?:boolean }) {
  ...
  return <div className="mx-auto max-w-3xl space-y-4">
    ...
    {liveVoiceEnabled&&<LiveVoicePanel history={history} contextId={work?.context.id??null} expectedRevision={work?.context.revision} onReply={applyVoiceReply} onWork={(next,note)=>{setWork(next);setMessage(note);request.current=null;mutation.current=null;}}/>}
    {voiceEnabled&&<details open={!liveVoiceEnabled}><summary className="cursor-pointer text-sm text-text-muted">기존 안정 음성 모드</summary><div className="mt-3"><VoicePanel history={history} contextId={work?.context.id??null} expectedRevision={work?.context.revision} automaticEnabled={automaticVoiceEnabled} onReply={applyVoiceReply} onWork={(next,note)=>{setWork(next);setMessage(note);request.current=null;mutation.current=null;}}/></div></details>}
