...
export function WorkWorkspace({ inlineApprovalsEnabled = false }: { inlineApprovalsEnabled?: boolean }) {
  ...
  return <div className="mx-auto max-w-3xl space-y-4">
    ...
    {preview&&<Card><h2 className="font-semibold">업무 저장 미리보기</h2>{([['goal','목표'],['progress','확인된 진행'],['nextStep','다음 행동']] as const).map(([key,label])=><label key={key} className="mt-3 block text-sm">{label}<textarea className={fieldClass} aria-label={label} rows={2} value={preview[key]} onChange={e=>setPreview({...preview,[key]:e.target.value})}/></label>)}
      ...
      <label className="mt-3 block text-sm"><input type="checkbox" checked={preview.deadlineReminder} onChange={e=>setPreview({...preview,deadlineReminder:e.target.checked})}/> 마감 24시간 전 알림 허용</label><label className="mt-2 block text-sm"><input type="checkbox" checked={preview.resumeReminder} onChange={e=>setPreview({...preview,resumeReminder:e.target.checked})}/> 진행 갱신 후 48시간 재개 알림 허용</label><p className="my-3 text-xs text-text-muted">조건 알림은 22–08 JST 보류·하루 최대 3개입니다. 직접 지정 시각은 예외입니다. 종료 후 맥락은 30일 보관하며 원문 대화는 저장하지 않습니다.</p><Button variant="primary" disabled={busy} onClick={()=>void run(savePreview)}>업무 저장</Button>
    </Card>}
