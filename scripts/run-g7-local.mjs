...
if (mode === "test" && args.some(path => ["tests/gates/g7-browser.test.ts", "tests/gates/g7-workflows-browser.test.ts"].includes(path))) env.G7_ALLOW_LIVE_AI = "1";
if (mode === "test" && args.includes("tests/gates/g7-workflows-browser.test.ts")) env.JARVIS_CALENDAR_ACTIONS_ENABLED = "true";
if (mode === "dev" && process.env.G7_CALENDAR_PROPOSALS === "1") env.JARVIS_CALENDAR_ACTIONS_ENABLED = "true";
