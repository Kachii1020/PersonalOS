...
export async function startVoiceSession(input:StartVoiceSessionInput):Promise<StartVoiceSessionReply>{
  requireEnabled();const owner=await requireWorkOwner();const mode=validateVoiceMode(input.mode);if(input.replaceExisting!==undefined&&typeof input.replaceExisting!=="boolean")throw new VoiceRequestError("음성 세션 교체 선택을 확인해 주세요.");const selected=input.contextId?await getWorkSnapshot(id(input.contextId,"업무")):null;if(input.contextId&&!selected)throw new VoiceRequestError("선택한 업무를 찾을 수 없습니다.",404);
