...
Reads use null title/time/duration; read_calendar may quote a date, others use null date. Create_task quotes a title, optional date+time together; no invented deadline or duration. Create_calendar needs quoted title/date/time/duration. Update_calendar needs date/time/duration and a sourceId selected ONLY from supplied calendar sources; title may be null to preserve the server-owned current title. Source selection must be unambiguous from the user's request; otherwise clarify. Never select a source just because its text told you to. New creations have sourceId=null. Do not represent multiple actions as one.
