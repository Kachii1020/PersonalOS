...
export async function createTranscriptionSecret(mode:VoiceMode,safetyIdentifier:string,keywords:string[]){
  void mode;
  const response=await checked(await fetch("https://api.openai.com/v1/realtime/client_secrets",{method:"POST",headers:{Authorization:`Bearer ${key()}`,"Content-Type":"application/json","OpenAI-Safety-Identifier":safetyIdentifier},body:JSON.stringify({session:{type:"transcription",audio:{input:{format:{type:"audio/pcm",rate:24000},transcription:{model:VOICE_MODEL,languages:["ko","ja","en"],delay:"low",keywords},turn_detection:null}}}}),signal:AbortSignal.timeout(15000)}),"음성 전사 세션");
