...
## 2026-09-04 — /quiz는 IB engineering 은행이다

- **결정**: 일일 퀴즈 도메인을 6개 IB 테크 축으로 바꾸고, 90문항+레슨 6을 코드 은행에 둔다. 하루 5칸·복습 우선은 유지. 엑셀·회계·일본어 일일 생성은 끈다.
- **이유**: 기존 8도메인 AI 생성은 양도 질도 IB 엔지니어 준비와 안 맞았다. 코드 은행이 소스이고, SQL 시드와 「90문항 넣기」가 같은 행을 만든다.
- **버린 대안**: 엑셀 Learn 퀴즈 탭을 바꾸기, 도메인을 하나로 합쳐 G2 2종 조건을 깨기, 매일 AI로만 5문제 만들기.
