...
## 2026-08-30 — IB 트랙은 시드 + 체크리스트

- **결정**: `ib-finance`는 `learn_*` 시드와 토픽 체크리스트다. Excel 트랙은 `lib/learn` 카드·랩을 유지한다. 토픽 완료는 스키마를 안 바꾸고 localStorage에 둔다. 빈 진행률 문구는 체크리스트에만 `첫 번째 토픽을 완료하면 진행률이 여기 표시됩니다`를 쓴다.
- **이유**: 개정안이 스키마 변경을 금지했고, Excel 실습 UI를 IB 토픽 줄에 끼우면 랩이 거짓말이 된다.
- **버린 대안**: Excel 커리큘럼을 DB로 옮기기, 토픽 완료용 새 테이블, 두 트랙을 한 카드 컴포넌트로 합치기.
...
## 2026-09-06 — Phase 5B 근거 기반 커리어 비서

- **결정**: 공개 공식 원문만 제한적으로 수집하고, AI는 미검토 조건 후보만 만든다. 개인 프로필은 서버의 결정론적 비교에만 쓰며 공식 근거·전체 조건 검토 없이는 지원 가능으로 확정하지 않는다.
- **이유**: 누락·만료·페이지 변경·수집 실패와 실제 지원 자격을 혼동하지 않고, 승인 기반 태스크 준비와 기록을 유지하기 위해서다. 소스/작업 commit은 revision과 lease를 함께 검사한다.
- **버린 대안**: 프로필의 AI 전송, 비공식 URL로 자격 확정, Notion 지원 기록 복제, 단일 요청의 수집·AI·외부 제출, 검증 미실행 상태에서의 운영 반영.

## 2026-09-07 — Phase 6 대화·승인·캘린더 실행

- **결정**: 모델은 사용자 원문 인용에 연결된 의도만 반환한다. 사실·시각·대상은 서버에서 검증하고, 불변 초안→명시적 pending 승인→별도 실행으로 분리한다. CalDAV는 승인당 한 번의 쓰기 권한과 읽기 전용 복구 영수증을 사용한다.
- **이유**: 유효한 인용만으로 자유형 답변의 진실성이 보장되지 않으며, 원격 PUT과 DB 완료는 하나의 트랜잭션이 아니다. 정기 동기화도 기존 앱 생성 출처를 보존해야 후속 UPDATE가 가능하다.
- **버린 대안**: 자유형 모델 답변을 실행 입력으로 사용, 응답 유실 시 무조건 PUT 재시도, UID 모양만으로 앱 출처 추정, 통과 증거 없이 운영 executor 활성화.

## 2026-09-07 — 고정 평가 기반 대화 보강

- **결정**: 100개 합성 사례의 정답을 먼저 고정하고 세 번의 실제 결과를 모두 보존한다. 조회 조건은 사용자 문장에서 결정론적으로 추출해 DB 제한보다 먼저 적용하고, 미지원 조건은 재질문한다.
- **이유**: 모델의 의도 분류가 맞아도 서버의 과도한 거부나 조건 생략으로 사용자 결과가 틀릴 수 있다. 취소/새 요청 경계와 문자 검색도 평가셋 밖의 독립 검사로 확인한다.
- **버린 대안**: 개선 후 정답을 바꿔 점수를 맞추기, 모델이 만든 필터를 그대로 SQL로 전달하기, 100/100 개발셋 결과를 일반 정확도 100%로 표현하기.
