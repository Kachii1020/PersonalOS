...
## 2026-08-03 — [승인 대기] 브리핑 모델을 claude-sonnet-5 / effort low로
...
- **결정**: 코드 기본값은 `claude-opus-5`로 두되(`AI_MODEL`로 재정의), 개발 환경은 `claude-sonnet-5` + `effort: "low"`로 돌린다.
- **이유**: Opus 5는 월 $8.19로 예산의 82%를 혼자 쓴다. SPEC 5.5는 AI 호출 지점이 3곳(브리핑·퀴즈·자료요약)이라고 명시하는데 Phase 2 몫이 남지 않는다. 두 출력을 직접 비교했고 같은 사건·같은 정확도였다. **어느 쪽을 기본값으로 할지는 승인 필요.**
