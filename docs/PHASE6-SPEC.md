...
2026-09-07 follow-up authorization: the user requested a fixed 100-case live evaluation and measurement-driven multi-turn/conditional-query improvements without a separate planning document. Bounded read conditions now include task status (open/done/all), explicit JST deadline date, due/priority ordering and a literal quoted title keyword; career conditions use existing derived eligibility or a literal title keyword. Apply conditions before the display limit, show the applied conditions and exact matching count, and clarify unknown/conflicting conditions rather than discard them. Calendar reads remain plain/date-scoped; unsupported modifiers clarify. Complete latest-user corrections may replace an earlier draft; old quotes or retained selection cannot revive a cancelled or superseded request. None of this authorizes automatic approval or additional action types.
