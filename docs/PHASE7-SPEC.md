...
- 60회 복원과 30개 독립 대화·실행 검사는 Phase 7 기능 인수 조건으로 유지한다. 실제 7일 정시성은 자동 조건 알림 승격 조건으로 분리하며, Phase 7 기능 완료나 음성 단계 착수를 막지 않는다.
- 제한 출시 또는 Phase 7 기능 완료는 정시성99% 달성을 뜻하지 않는다.
...
- [x] G7A-3 DB projection 60/60 및 실제 iPhone→Mac 동일 업무/revision 복원 1회 통과
...
- [ ] G7B-3 자동 조건 알림 승격용 실제7일 정시성 ≥99% — 2026-09-09 10:20 JST 시작; Phase 7 기능/음성 단계와 분리, 통과 전 자동 조건 알림 ON 금지
- [x] G7C-1 대화내 개별승인/결과/partial 구현, 실모델 브라우저 task 승인 및 DB 합성 partial 검사
- [x] G7C-2 고정 v2 30개를 strict deterministic system path로 모두 검증: 전체 실행29/30 뒤 일시적 DB timeout C04 재실행1/1, 합계30/30; 모델호출·duplicate·미승인쓰기·거짓완료·외부캘린더쓰기0
- [x] 기존 회귀/관련 DB·브라우저 실행, 보고서 및 PR #30 (259 unit, 기존 100/100 실모델 개발셋 재사용; 외부 게이트 제외는 보고서 참조)

배포와게이트 상태는 PHASE7-REPORT.md에 실제출력만 기록한다. Phase 7 기능완료와 자동조건알림의7일운영인수완료는 별도 판정한다. 이미허용한반복운영질문/teach-back 생략은권한검증생략을뜻하지않는다.
