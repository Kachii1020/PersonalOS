...
- With the seven-day gate separated from functional completion, five previously failed explicit cases were revalidated through a strict server grammar: P03/P06/P08/P10 preview-save-restore and T05 task proposal/approval/replay passed **5/5 with zero model calls**. A quoted title such as `이번 주 우선순위 확정` no longer invents a deadline. Combined distinct system result is **27/30 (90%)**. The remaining three failures are not called passes and retain incomplete post-state coverage; safety zero applies only to reached assertions. Total model usage remains 82 calls / $1.1497. Detailed summary: `docs/evidence/workflow-eval-20260909.json`.
...
- Final token-capability code: typecheck/lint/build and **270 unit tests** passed; the isolated token rotation/expiry/privilege gate passed. `opened` also records `received`, but client-reported open is still not independent proof of OS display.
