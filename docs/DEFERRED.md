...
- **G4 실기기** — 푸시 구독 왕복(조건 1)과 오프라인 폴백(조건 4·5)은 홈 화면에 설치한 iPhone에서 확인해야 한다. 순서는 `docs/SHIP.md` 3절.
- **VAPID 키** — 로컬에서 `npm run vapid:generate`. 같은 두 값을 Vercel에 넣고 리포에 넣지 않는다. 순서는 `docs/SHIP.md` 1절.
