...
- **호스티드 0011+0012** — `supabase/migrations/0011_sec_filings.sql` 및 `0012_learn_module.sql`을 `db push`로 올려야 한다.
- **Learn 진행률 Supabase 연동** — 현재 퀴즈 진행률은 React state(페이지 새로고침 시 초기화). learn_progress 테이블에 저장하려면 서버 액션 연결 필요.
- **Learn 퀴즈 → SR 복습 큐 합류** — /learn에서 푼 문제를 quiz_attempts에 기록하고 오답을 quiz_review_queue에 추가하는 연동.
