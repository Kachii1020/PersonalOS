...
- ~~**강의자료 원본 열기·삭제**~~ — 2-E에서 구현. 60초 서명 URL + 삭제(스토리지 파일까지).
...
- **G4 조건 8** — 예산 0에서 주간 리뷰 → 402. 2·6은 2026-08-20 라이브 통과. `docs/SHIP.md` 4절 5항.

## 업그레이드 계획 (docs/UPGRADE-PLAN.html)에서 미룬 것

- **Tier 3 전체 (3-A 적응형 퀴즈 / 3-B 행동 제안 / 3-C 자연어 인터페이스)** — 계획서 자신의 게이트: 실데이터 2주 축적 후. 3-C는 SPEC 5.5에 6번째 호출 지점(`intent_parse`) 개정 승인이 먼저다.
- **1-D 아침 루틴 화면** — 계획서가 선택 사항으로 표시. 실사용 1주일(0-C) 후 판단.
- **주간 뷰 이벤트 상세 팝업** — 2-A는 블록 + title 툴팁까지. 클릭 상세·삭제는 월간 뷰에 있다.
- **주간 뷰 겹침 열 분할** — 겹치는 이벤트는 살짝 오른쪽으로 미는 것까지. 열 분할 레이아웃은 겹침이 실제로 잦아지면.
