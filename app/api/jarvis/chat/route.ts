...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const status = error instanceof DialogueRequestError ? error.status : error instanceof SyntaxError ? 400 : error instanceof Error && error.name === "BudgetExceededError" ? 402 : 503;
    ...
    return NextResponse.json({ error: error instanceof DialogueRequestError ? error.message : status === 402 ? "AI 예산을 모두 사용했습니다. 추가 실행은 하지 않았습니다." : "대화 요청을 처리하지 못했습니다. 잠시 후 다시 시도하세요." }, { status });
  }
}
