...
export default function JarvisPage() {
  ...
  return <><WorkWorkspace inlineApprovalsEnabled={process.env.JARVIS_INLINE_APPROVALS_ENABLED === "true"} automaticAttentionEnabled={process.env.JARVIS_AUTOMATIC_ATTENTION_ENABLED === "true"} voiceEnabled={process.env.JARVIS_VOICE_ENABLED === "true"} automaticVoiceEnabled={process.env.JARVIS_VOICE_AUTO_TURN_ENABLED === "true"}/><details className="mx-auto mt-6 max-w-3xl"><summary className="cursor-pointer text-sm text-text-muted">업무와 무관한 기존 단건 대화</summary><div className="mt-4"><JarvisConversation/></div></details></>;
}
